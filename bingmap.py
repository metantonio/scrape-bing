...
# Abre Excel file
file_path = "./Casino_List_9_13.xlsx"
...
for index, row in df.iterrows():
    new_latitude = row['New Latitude']
    new_longitude = row['New Longitude']
    state = row['State']
    city = row['City']
    checked = row['CheckedCoord']
    if checked != "ok":
        # Abre Google Maps y busca el casino
        #url = f"https://www.google.com/maps/search/{name}+{city}+{state}"
        ...
        try:           
            ...
            #print(row)
            #print(f"Name: {row['Name']}, State: {row['State']}, Latitude: {latitude}, Longitude: {longitude}, AG: {row['AG']}")
